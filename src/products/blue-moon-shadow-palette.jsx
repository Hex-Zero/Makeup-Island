...
const BlueMoonShadowPalette = () => {
  const [state, setState] = useState([])
  const [info, setInfo] = useState({})
  const data = useStaticQuery(graphql`
    query {
      allMongodbMakeupIslandProducts(
        filter: { local: { eq: "blue-moon-shadow-palette" } }
      ) {
        edges {
          node {
            title
      ...
      allFile(
        filter: {
          relativeDirectory: { eq: "products/blue-moon-shadow-palette" }
        }
      ) {
        edges {
          node {
...
export default BlueMoonShadowPalette
