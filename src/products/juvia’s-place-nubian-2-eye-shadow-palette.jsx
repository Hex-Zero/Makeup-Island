...
const JuviaPlaceNubian2 = () => {
  const [state, setState] = useState([])
  const [info, setInfo] = useState({})
  const data = useStaticQuery(graphql`
    query {
      allMongodbMakeupIslandProducts(
        filter: { local: { eq: "juvia’s-place-nubian-2-eye-shadow-palette" } }
      ) {
        edges {
          node {
            title
      ...
      allFile(
        filter: {
          relativeDirectory: {
            eq: "products/juvia’s-place-nubian-2-eye-shadow-palette"
          }
        }
      ) {
        edges {
...
export default JuviaPlaceNubian2
