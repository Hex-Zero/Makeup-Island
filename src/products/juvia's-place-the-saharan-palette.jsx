...
const JuviaPlaceTheSaharan = () => {
  const [state, setState] = useState([])
  const [info, setInfo] = useState({})
  const data = useStaticQuery(graphql`
    query {
      allMongodbMakeupIslandProducts(
        filter: { local: { eq: "juvia's-place-the-saharan-palette" } }
      ) {
        edges {
          node {
            title
      ...
      allFile(
        filter: {
          relativeDirectory: {
            eq: "products/juvia's-place-the-saharan-palette"
          }
        }
      ) {
        edges {
...
export default JuviaPlaceTheSaharan
