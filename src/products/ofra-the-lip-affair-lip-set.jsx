...
const OfraTheLipAffairLipSet = () => {
  const [state, setState] = useState([])
  const [info, setInfo] = useState({})
  const data = useStaticQuery(graphql`
    query {
      allMongodbMakeupIslandProducts(
        filter: { local: { eq: "ofra-the-lip-affair-lip-set" } }
      ) {
        edges {
          node {
            title
      ...
      allFile(
        filter: {
          relativeDirectory: { eq: "products/ofra-the-lip-affair-lip-set" }
        }
      ) {
        edges {
          node {
...
export default OfraTheLipAffairLipSet
