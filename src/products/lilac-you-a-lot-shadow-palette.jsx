...
const LilacYouALotShadoPalette = () => {
  const [state, setState] = useState([])
  const [info, setInfo] = useState({})
  const data = useStaticQuery(graphql`
    query {
      allMongodbMakeupIslandProducts(
        filter: { local: { eq: "lilac-you-a-lot-shadow-palette" } }
      ) {
        edges {
          node {
            title
      ...
      allFile(
        filter: {
          relativeDirectory: { eq: "products/lilac-you-a-lot-shadow-palette" }
        }
      ) {
        edges {
          node {
...
export default LilacYouALotShadoPalette
