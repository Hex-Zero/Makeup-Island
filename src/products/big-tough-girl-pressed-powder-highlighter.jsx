...
const BigToughGirlPressedPowderHighlighter = () => {
  const [state, setState] = useState([])
  const [info, setInfo] = useState({})
  const data = useStaticQuery(graphql`
    query {
      allMongodbMakeupIslandProducts(
        filter: { local: { eq: "big-tough-girl-pressed-powder-highlighter" } }
      ) {
        edges {
          node {
            title
      ...
      allFile(
        filter: {
          relativeDirectory: {
            eq: "products/big-tough-girl-pressed-powder-highlighter"
          }
        }
      ) {
        edges {
...
export default BigToughGirlPressedPowderHighlighter
