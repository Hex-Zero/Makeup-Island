...
const UhhuhHoneyShadowPalette = () => {
  const [state, setState] = useState([])
  const [info, setInfo] = useState({})
  const data = useStaticQuery(graphql`
    query {
      allMongodbMakeupIslandProducts(
        filter: { local: { eq: "uhhuh-honey-shadow-palette" } }
      ) {
        edges {
          node {
            title
      ...
      allFile(
        filter: {
          relativeDirectory: { eq: "products/uhhuh-honey-shadow-palette" }
        }
      ) {
        edges {
          node {
...
export default UhhuhHoneyShadowPalette
