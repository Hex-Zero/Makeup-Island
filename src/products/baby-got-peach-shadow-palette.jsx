...
const BabyGotPeachShadoPalette = () => {
  const [state, setState] = useState([])
  const [info, setInfo] = useState({})
  const data = useStaticQuery(graphql`
    query {
      allMongodbMakeupIslandProducts(
        filter: { local: { eq: "baby-got-peach-shadow-palette" } }
      ) {
        edges {
          node {
            title
      ...
      allFile(
        filter: {
          relativeDirectory: { eq: "products/baby-got-peach-shadow-palette" }
        }
      ) {
        edges {
          node {
...
export default BabyGotPeachShadoPalette
