...
const LoveBirdShadowPalette = () => {
  const [state, setState] = useState([])
  const [info, setInfo] = useState({})
  const data = useStaticQuery(graphql`
    query {
      allMongodbMakeupIslandProducts(
        filter: { local: { eq: "love-bird-shadow-palette" } }
      ) {
        edges {
          node {
            title
      ...
      allFile(
        filter: {
          relativeDirectory: { eq: "products/love-bird-shadow-palette" }
        }
      ) {
        edges {
          node {
...
export default LoveBirdShadowPalette
